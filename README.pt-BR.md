...
2. Extraia o adaptador para `C:\NaturalVoiceSAPIAdapter` (local recomendado)
3. Abra o instalador e **clique em ambos os botões "Install 32-bit" e "Install 64-bit"** (isso é obrigatório!)
4. Instale as vozes localmente (recomendado):
   ...
   - Crie uma pasta `Voices` dentro da pasta do adaptador (ex: `C:\NaturalVoiceSAPIAdapter\Voices`)
   - Extraia cada arquivo MSIX (como um ZIP) para sua própria subpasta dentro da pasta Voices
   - No instalador do NaturalVoiceSAPIAdapter, defina `C:\NaturalVoiceSAPIAdapter\Voices` como "Local voice path"
